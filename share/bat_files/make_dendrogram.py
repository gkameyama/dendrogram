...
def create_dendrogram(
    ...
    output_path_arg: Path | None = None,
    matrix_path_arg: Path | None = None,
) -> dict[str, Path | str]:
    excel_path = resolve_excel_path(input_path)
    output_path, timestamp = build_output_path(excel_path, output_path_arg)
    matrix_path = build_matrix_output_path(excel_path, matrix_path_arg, timestamp)
    set_japanese_font()
...
def main() -> None:
    args = parse_args()
    result = create_dendrogram(args.input, args.output, args.output_matrix)

    print(f"input  : {result['input']}")
    print(f"data   : {result['data_sheet']}")
    print(f"label  : {result['label_sheet']}")
